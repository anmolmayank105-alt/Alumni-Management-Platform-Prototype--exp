...
class MongoAlumniDatabase {
  ...
  private async initialize() {
    // Give MongoDB service time to initialize
    await new Promise(resolve => setTimeout(resolve, 100));
    
    const connected = await checkMongoConnection();
    if (connected) {
      console.log('🎓 Alumni Management Database initialized successfully');
      console.log('📊 Connection info:', mongodb.getConnectionInfo());
    } else {
      console.warn('⚠️ Database initialization issue, retrying...');
      // Retry after a short delay
      setTimeout(async () => {
        const retryConnected = await checkMongoConnection();
        if (retryConnected) {
